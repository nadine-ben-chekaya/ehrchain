...
app.use((req, res, next) => {
    ...
    if (req.originalUrl.indexOf('/users') >= 0 || req.originalUrl.indexOf('/users/login') >= 0 || req.originalUrl.indexOf('/register') >= 0) {
        //will not have jwt so it will go to the main fnct (ma3anech token)
        return next();
    }
    //verify token
    var token = req.token;
    jwt.verify(token, app.get('secret'), (err, decoded) => {
        if (err) {
            ...
            //if verification success then it will go to the fonction
...
// Register and enroll user to the hospital fabric network
